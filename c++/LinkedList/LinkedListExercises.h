...
void LinkedList<T>::insertOrdered(const T& newData) {
  ...
  if (!head_ || (newData < (head_ -> data))) {
    ...
    Node *thru = head_;
    T thru_data = thru -> data;

    while (newData > thru_data){
      thru = thru -> next;
      thru_data = thru ->data;
      // stop at the thru pointing to the position we want to insert before
    }

    Node* newNode = new Node(newData);
    Node* Nodeafter = thru;
    Node* Nodebefore = thru -> prev;
...
LinkedList<T> LinkedList<T>::merge(const LinkedList<T>& other) const {
  ...
  if (!head_){
    ...
    Node* left_thru = left.getHeadPtr();
    Node* right_thru = right.getHeadPtr();
    T left_data = left_thru -> data;
    T right_data = right_thru -> data;

    while ((left_thru != nullptr) && (right_thru != nullptr)){
      left_data = left_thru -> data;
      right_data = right_thru -> data;
    ...
    //some thru reaches nullptr
    if (left_thru == nullptr){
      while (right_thru != nullptr){
        right_data = right_thru -> data;
        merged.pushBack(right_data);
        right_thru = right_thru -> next;
      }
